...
int main( int argc, char** argv )
{
	...
	while(1)
    {
		for(t=0;t<N;t++) //Start computing saliency map (up to N frames)
		{
			...
			/*Computing motion vectors for each block of the frame with respect the previous frame t-1*/
			if(t > 0)
			{
				...
				for(i=0;i<patch_y;i++)
					for(j=0;j<patch_x;j++)
					{
						...
						theta[i][j][t-1] = atan2((double)MVy[i][j],(double)MVx[i][j]);
					}
			}

			if(t == 14) // Compute maps
			{
		...
		cout<<C[0][19]<<","<<Ment[0][19]<<","<<Dent[0][19]<<endl;
